...
@pytest.mark.parametrize("major", MAJORS)
def test_no_nested_version_tree(major: str) -> None:
    """
    A version package must not contain another version's bindings.

    `codegen generate --version-dir` writes into `<outpath>/v<major>/`. Pointing
    it at a version package's own src/ffmpeg therefore nests a whole second set
    of bindings inside it, and setuptools' `include = ["ffmpeg*"]` ships them:
    typed-ffmpeg-v6 4.4 carried `packages/v6/src/ffmpeg/v8/`, so `import
    ffmpeg.v8` worked and handed a v6 user FFmpeg 8 bindings.
    """
    ffmpeg_dir = ROOT / "packages" / f"v{major}" / "src" / "ffmpeg"
    nested = sorted(
        p.name
    ...
    assert not nested, (
        f"packages/v{major}/src/ffmpeg contains nested version bindings {nested}; "
        "these ship with the package. Generated by --version-dir pointed at the "

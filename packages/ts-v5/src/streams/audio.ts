 ...
 */
  acrossfade(
    ...
    duration?: FFDuration;
    overlap?: FFBoolean;
    curve1?: FFInt | "nofade" | "tri" | "qsin" | "esin" | "hsin" | "log" | "ipar" | "qua" | "cub" | "squ" | "cbr" | "par" | "exp" | "iqsin" | "ihsin" | "dese" | "desi" | "losi" | "sinc" | "isinc";
    curve2?: FFInt | "nofade" | "tri" | "qsin" | "esin" | "hsin" | "log" | "ipar" | "qua" | "cub" | "squ" | "cbr" | "par" | "exp" | "iqsin" | "ihsin" | "dese" | "desi" | "losi" | "sinc" | "isinc";
extraOptions?: Record<string, unknown>;
    },
  ): AudioStream {
    const filterNode = filterNodeFactory(
 ...
 *
 * @param options.threshold - Set the detection threshold used to trigger equalization. Threshold detection is using bandpass filter. Default value is 0. Allowed range is from 0 to 100.
 * @param options.dfrequency - Set the detection frequency in Hz used for bandpass filter used to trigger equalization. Default value is 1000 Hz. Allowed range is between 2 and 1000000 Hz.
 * @param options.dqfactor - Set the detection resonance factor for bandpass filter used to trigger equalization. Default value is 1. Allowed range is from 0.001 to 1000.
 ...
 * @param options.knee - Curve the sharp knee around the detection threshold to calculate equalization gain more softly. Default is 1. Allowed range is between 0 and 8.
 * @param options.ratio - Set the ratio by which the equalization gain is raised. Default is 1. Allowed range is between 1 and 20.
 * @param options.makeup - Set the makeup offset in dB by which the equalization gain is raised. Default is 0. Allowed range is between 0 and 30.
 * @param options.range - Set the max allowed cut/boost amount in dB. Default is 0. Allowed range is from 0 to 200.
 * @param options.slew - Set the slew factor. Default is 1. Allowed range is from 1 to 200.
 * @param options.mode - Set the mode of filter operation, can be one of the following: @end table Default mode is cut.
 ...
 */
  afade(
    ...
    duration?: FFDuration;
    curve?: FFInt | "nofade" | "tri" | "qsin" | "esin" | "hsin" | "log" | "ipar" | "qua" | "cub" | "squ" | "cbr" | "par" | "exp" | "iqsin" | "ihsin" | "dese" | "desi" | "losi" | "sinc" | "isinc";
    enable?: FFString;
extraOptions?: Record<string, unknown>;
    },
  ): AudioStream {
...
extraOptions?: Record<string, unknown>;
    ...
    const filterNode = filterNodeFactory(
      { name: "afifo", typingsInput: ["audio"], typingsOutput: ["audio"] },
      [this],
      merge(
    {
},
 ...
 */
  aformat(
    options?: {
    sample_fmts?: FFString;
    sample_rates?: FFString;
 ...
 * @param options.mode - set mode (from 0 to 1) (default full)
 * @param options.flags - set flags (default queue)
 * @param options.rate - set video rate (default "25")
 ...
 * @param options.precision - Set precison of filtering. @end table
 ...
 * @param options.level - Set input volume level. Allowed range is from 0 to 4. Defalt is 1.
 ...
 * @param options.algo - Set algorithm for cross-correlation. Can be slow or fast. Default is slow. Fast algorithm assumes mean values over any given segment are always zero and thus need much less calculations to make. This is generally not true, but is valid for typical audio streams.
 ...
 * @param options.channel_layout - The channel layout of the output stream.
...
/**
 * Simple algorithm for audio noise sharpening. This filter linearly increases differences betweeen each audio sample. The filter accepts the following options:
 ...
 * @param options.overlap - Set ratio of overlap window. Default value is 0.5. When value is 1 overlap is set to recommended size for specific window function currently used.
 ...
 * @param options.start_mode - Specify mode of detection of silence end in start of multi-channel audio. Can be any or all. Default is any. With any, any sample that is detected as non-silence will cause stopped trimming of silence. With all, only if all channels are detected as non-silence will cause stopped trimming of silence.
 * @param options.stop_periods - Set the count for trimming silence from the end of audio. To remove silence from the middle of a file, specify a stop_periods that is negative. This value is then treated as a positive value and is used to indicate the effect should restart processing as specified by start_periods, making it suitable for removing periods of silence in the middle of the audio. Default value is 0.
 ...
 * @param options.stop_mode - Specify mode of detection of silence start in end of multi-channel audio. Can be any or all. Default is any. With any, any sample that is detected as non-silence will cause stopped trimming of silence. With all, only if all channels are detected as non-silence will cause stopped trimming of silence.
 * @param options.detection - Set how is silence detected. Can be rms or peak. Second is faster and works better with digital silence which is exactly 0. Default value is rms.
 * @param options.window - Set duration in number of seconds used to calculate size of window in number of samples for detecting silence. Default value is 0.02. Allowed range is from 0 to 10.

...
def acrossfade(
    ...
    *streams: VideoStream,
    ...
    inputs: Int = Default('2'),nb_samples: Int64 = Default('44100'),duration: Duration = Default('0'),overlap: Boolean = Default('true'),curve1: Int| Literal["nofade","tri","qsin","esin","hsin","log","ipar","qua","cub","squ","cbr","par","exp","iqsin","ihsin","dese","desi","losi","sinc","isinc","quat","quatr","qsin2","hsin2"] | Default = Default('tri'),curve2: Int| Literal["nofade","tri","qsin","esin","hsin","log","ipar","qua","cub","squ","cbr","par","exp","iqsin","ihsin","dese","desi","losi","sinc","isinc","quat","quatr","qsin2","hsin2"] | Default = Default('tri'),
...
Args:
    inputs: Specify the number of inputs to crossfade. When crossfading multiple inputs, each input will be concatenated and crossfaded in sequence, similar to the concat filter. Default is 2.
...
References:
    ...
    filter_node = filter_node_factory(
        FFMpegFilterDef(name='acrossfade', typings_input=(), typings_output=('audio',)),
...
Args:
    ...
    taps: Set the number of filter coefficients in output audio stream. Default value is 0.
...
Args:
    ...
    taps: Set number of filter coefficients in output audio stream. Default value is 4096.
...
Args:
    taps: Set number of filter coefficients in output audio stream. Default value is 1025.
...
def amerge(
    ...
    *streams: AudioStream,
    ...
    inputs: Int = Auto('len(streams)'),layout_mode: Int| Literal["legacy","reset","normal"] | Default = Default('legacy'),
...
Args:
    ...
    layout_mode: This option controls how the output channel layout is determined and if the audio channels are reordered during merge. @end table
    extra_options: Extra options for the filter
...
Args:
    ...
    colorspace: A string representing the color space of the buffered video frames. It may be a number corresponding to a color space, or a color space name.
    range: A string representing the color range of the buffered video frames. It may be a number corresponding to a color range, or a color range name.
    alpha_mode: A string representing the alpha mode of the buffered video frames. It may be a number corresponding to an alpha mode, or an alpha mode name.
    extra_options: Extra options for the filter
...
Args:
    ...
    type: Set type of gradients. Available values are: @end table Default type is linear.
    extra_options: Extra options for the filter
...
Args:
    size: Specify the size (width and height) of the buffered video frames. For the syntax of this option, check the "Video size" section in the ffmpeg-utils manual. Default value is 320x240.
    rate: Specify the frame rate expected for the video stream, expressed as a number of frames per second. Default value is 25.
    octaves: Specify the total number of components making up the noise, each one with doubled frequency. Default value is 1.
    persistence: Set the ratio used to compute the amplitude of the next octave component with respect to the previous component amplitude. Default value is 1.
    xscale: set x-scale factor (from 0 to DBL_MAX) (default 1)
    yscale: Define a scale factor used to multiple the x, y coordinates. This can be useful to define an effect with a pattern stretched along the x or y axis. Default value is 1.
    tscale: Define a scale factor used to multiple the time coordinate. This can be useful to change the time variation speed. Default value is 1.
    random_mode: Set random mode used to compute initial pattern. Supported values are: @end table Default value is random.
    random_seed: When random_mode is set to random_seed, use this value to compute the initial pattern. Default value is 0.
    extra_options: Extra options for the filter
